...
export default function LoginPage() {
  ...
  async function handleMagicLinkLogin(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess(
        "Der Anmelde-Link wurde verschickt. Bitte prüfe jetzt dein E-Mail-Postfach."
      );
      setEmail("");
    } catch (err) {
      setError(
            ...
            <form onSubmit={handleMagicLinkLogin} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none transition placeholder:text-white/35 focus:border-orange-400/40"
                />
              </div>
